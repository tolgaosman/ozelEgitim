...
export async function generateMetadata({ params }: ProgramDetailPageProps): Promise<Metadata> {
  ...
  if (!program) {
    const content = await fetchPageContentBlock("program_detail");
  ...
  return {
    ...
    alternates: { canonical: `/programlar/${program.slug}` },
  };
}
...
export default async function ProgramDetailPage({ params }: ProgramDetailPageProps) {
  ...
  const [program, content] = await Promise.all([
    ...
    fetchPageContentBlock("program_detail"),
  ]);
  ...
  return (
    <>
      <section className="relative overflow-hidden bg-navy-900" data-scrim>
        <Image src={resolveProgramImage(program.slug, program.image)} alt="" fill preload sizes="100vw" className="object-cover" />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    {program.highlights.map((highlight, idx) => (
                      <div
                        key={idx}
                        className="group flex flex-col items-start gap-4 rounded-2xl border border-transparent bg-paper p-6 transition-all duration-300 hover:border-aqua-500/30 hover:bg-white hover:shadow-[var(--shadow-card)]"
                      >
                ...
                <div className="relative z-10">
                  ...
                  <dl className="space-y-6">
                    <div className="flex flex-col gap-1.5 border-b border-white/10 pb-6">
                      <dt className="text-xs font-bold text-white/50 uppercase tracking-widest">{content.ageLabel}</dt>
                      <dd className="text-lg font-medium text-white">{program.ageRangeLabel}</dd>
                    </div>
                    <div className="flex flex-col gap-1.5 pb-2">
                      <dt className="text-xs font-bold text-white/50 uppercase tracking-widest">{content.formatLabel}</dt>
