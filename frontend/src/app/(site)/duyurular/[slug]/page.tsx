...
export default async function AnnouncementDetailPage({ params }: AnnouncementDetailPageProps) {
  const { slug } = await params;
  const [announcement, content] = await Promise.all([
    fetchAnnouncementBySlug(slug),
    fetchPageContentBlock("announcement_detail"),
  ]);
  ...
  return (
    <>
      <PageHero
        lead={content.breadcrumbLead}
        accent={content.breadcrumbAccent}
            ...
            <div className="sticky top-32 space-y-8">
              ...
              <Reveal delaySeconds={0.1}>
                <div className="rounded-xl border border-navy-800/10 bg-accent p-6 shadow-[var(--shadow-card)]">
                  <h3 className="font-display text-lg font-bold text-navy-800">{content.sidebarTitle}</h3>
                        ...
                        <li key={item.id} className="group">
                          <Link href={`/duyurular/${item.slug}`} className="block">
                            ...
                            <div className="mt-2 flex items-center text-[13px] font-medium text-ink-soft group-hover:text-signal transition-colors">
                              {content.otherItemCtaLabel} <ChevronRight className="ml-1 size-3" />
                            </div>
                          </Link>
                        </li>
                      ))}
